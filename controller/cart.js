  ...
  try {
    ...
    if (cartItem) {
      ...
      cartItem.Total_price =
        parseInt(variation.price) * cartItem.quantity;
      ...
      Cart.items.push({
        ...
        variation: variation,
        quantity: quantity,
        priceWithTax: variation.price - variation.discounted_price
    ...
    Cart.items.forEach(item => {
      ...
      console.log('parseFloat(item.tax)....', parseFloat(item.tax))

      const taxRate = parseFloat(item.tax) * item.Total_price / 100;



    item.priceWithTax = taxRate + item.Total_price
      item.totalPriceWithDiscount = item.priceWithTax - item.Total_discounted_price
    });
    await Cart.save();
    ...
    if (Cart) {
      ...
      if (cartItemIndex !== -1) {
        ...
        if (quantity >= 0) {
          ...
          updatedTotalPrice = Cart.items[cartItemIndex].variation.price * quantity;
          updatedDiscountPrice = Cart.items[cartItemIndex].variation.discounted_price * quantity;
          updatedFinalAmount = updatedTotalPrice - updatedDiscountPrice;
        }
        ...
        Cart.items.forEach((item) => {
          const taxRate = parseFloat(item.tax) * item.totalPriceWithDiscount / 100;
          item.priceWithTax = taxRate + item.totalPriceWithDiscount;
          item.totalPriceWithDiscount = item.priceWithTax - item.Total_discounted_price;
        });

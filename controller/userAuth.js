...
async function userLogin(req, res) {
  try {
    ...
    if (email != "" && password != "") {
      ...
      userRes = await users.findOne({ email: email });
      if(userRes){

      
      if (userRes.role == 'deliveryboy') {
        let deliveryboyDetails = await deliveryboy.findOne({ email: email });
        ...
        loginMesaage = "successfully login deliveryboy"

        console.log('userData,,,',userData)
      }
      else if (userRes.role == 'user') {
        ...
        loginMesaage = "user login successfully"
